...
namespace HierarchyPro
{
    public class GroupSelection : IHierarchyShelf
    {
        ...
        public VisualElement CreateShelfElement()
        {
            ...
            Label option1 = new Label("相同父对象坐标系 (Offset)");
            ...
            option1.RegisterCallback<MouseDownEvent>((evt) =>
            {
                if (Selection.gameObjects.Length > 0)
                {
                    if (IsSelectionsSameScene())
                    {
                        ...
                        {
                            Debug.LogWarning("无法将选择的父组分组，请使用全局");
                        }
                    }
                    else
                    {
                        Debug.LogWarning("无法将多个场景中的选择分组.");
                    }
                }
            ...
            Label option2 = new Label("全局坐标系 (Offset)");
            ...
            Label option3 = new Label("全局原点坐标系 (Zero)");
