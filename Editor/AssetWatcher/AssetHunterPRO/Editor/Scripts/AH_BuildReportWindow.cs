...
namespace TransformPro.AssetHunterPRO
{
    public class AH_BuildReportWindow : EditorWindow
    {
        ...
        [MenuItem("Window/构建/资源 查看器/打开 构建报告")]
        public static void Init()
        {
            ...
            if(!alreadyExist)
            { 
                m_window = GetWindow<AH_BuildReportWindow>("构建报告", true, typeof(AH_Window));
        ...
        void OnGUI()
        {
            if (!m_window)
                Init();
            Heureka_WindowStyler.DrawGlobalHeader(m_window, AH_EditorData.Instance.WindowHeaderIcon.Icon, Heureka_WindowStyler.clr_Dark, "构建报告", AH_Window.VERSION);

            if (buildInfoManager == null || buildInfoManager.HasSelection == false)
            {
                Heureka_WindowStyler.DrawCenteredMessage(m_window, AH_EditorData.Instance.WindowHeaderIcon.Icon, 310f, 110f, "当前主窗口中没有加载构建信息");
                return;
            }
            else if (buildInfoManager.IsMergedReport())
            {
                Heureka_WindowStyler.DrawCenteredMessage(m_window, AH_EditorData.Instance.WindowHeaderIcon.Icon, 366f, 110f, "构建报告窗口 不能与合并构建报告一起工作");
                return;
            }
        ...
        private class AH_BuildReportWindowData
        {
            ...
            internal void OnGUI()
            {
                if (buildSize <= 0)
                {
                    Heureka_WindowStyler.DrawCenteredMessage(m_window, AH_EditorData.Instance.WindowHeaderIcon.Icon, 462f, 120f, "选择的构建信息缺少关键信息。它可能是用旧版本创建的。请用这个版本创建新的");
                    return;
                }

                int guiWidth = 260;
                EditorGUILayout.BeginHorizontal();
                EditorGUILayout.LabelField(" 合计构建大小:", Heureka_EditorData.Instance.HeadlineStyle, GUILayout.Width(guiWidth));
                EditorGUILayout.LabelField(AH_Utils.GetSizeAsString(buildSize), Heureka_EditorData.Instance.HeadlineStyle);
                GUILayout.FlexibleSpace();
                EditorGUILayout.EndHorizontal();
                EditorGUILayout.BeginHorizontal();
                EditorGUILayout.LabelField(" 构建平台:", Heureka_EditorData.Instance.HeadlineStyle, GUILayout.Width(guiWidth));
                EditorGUILayout.LabelField(buildTarget, Heureka_EditorData.Instance.HeadlineStyle);
                GUILayout.FlexibleSpace();
                EditorGUILayout.EndHorizontal();
                EditorGUILayout.BeginHorizontal();
                EditorGUILayout.LabelField(" 构建时间:", Heureka_EditorData.Instance.HeadlineStyle, GUILayout.Width(guiWidth));
