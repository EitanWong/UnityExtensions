...
using System;
using System.Linq;
using TransformPro.AssetHunterPRO.BaseTreeviewImpl;
using TransformPro.AssetHunterPRO.BaseTreeviewImpl.AssetTreeView;
...
namespace TransformPro.AssetHunterPRO
{
    public class AH_Window : EditorWindow
    {
        ...
        [UnityEditor.MenuItem("Window/构建/资源 查看器/打开 资源查看器 ^%h", priority = WINDOWMENUITEMPRIO)]
        ...
        {
            titleContent = new GUIContent("资源 查看器", AH_EditorData.Instance.WindowPaneIcon.Icon);

            guiContentLoadBuildInfo = new GUIContent("加载", AH_EditorData.Instance.LoadLogIcon.Icon,
                "从以前的构建中加载信息");
            guiContentGenerateBuildInfo = new GUIContent("构建", AH_EditorData.Instance.GenerateIcon.Icon,
                "手动创建虚拟构建信息。需要在构建设置中启用场景");
            guiContentSettings = new GUIContent("设置", AH_EditorData.Instance.Settings.Icon, "打开设置");

            //Only avaliable in 2018
#if UNITY_2018_1_OR_NEWER
            guiContentBuildReport = new GUIContent("报告", AH_EditorData.Instance.ReportIcon.Icon,
                "构建报告查看(构建包体大小信息)");
#endif
            guiContentReadme = new GUIContent("帮助", AH_EditorData.Instance.HelpIcon.Icon,
                "打开帮助文档");
            guiContentDeleteAll = new GUIContent("删除所有", AH_EditorData.Instance.DeleteIcon.Icon,
                "删除所有项目中未使用的资源 ({0}) 记得在设置窗口中手动排除相关资源");
            guiContentRefresh = new GUIContent(AH_EditorData.Instance.RefreshIcon.Icon, "刷新数据");
        }
        ...
        private void doNoBuildInfoLoaded()
        {
            Heureka_WindowStyler.DrawCenteredMessage(m_window, AH_EditorData.Instance.WindowHeaderIcon.Icon, 380f, 110f,
                "构建信息未加载" + Environment.NewLine + "您可以：加载现有的 或 构建新的");
        }
        ...
        private void doHeader()
        {
            Heureka_WindowStyler.DrawGlobalHeader(m_window, AH_EditorData.Instance.WindowHeaderIcon.Icon,
                Heureka_WindowStyler.clr_Pink, "资源 查看器", VERSION);
            ...
            if (infoLoaded)
            {
                ...
                ) // GUILayout.Button(content, GUILayout.MaxWidth(32), GUILayout.Height(18)))
                    RefreshBuildLog();
            ...
            EditorGUI.BeginDisabledGroup(
                !EditorBuildSettings.scenes.Any(val =>
                    val.enabled == true)); //Disable the generate btn if there are no enabled scenes in buildsettings
            ...
            GUIContent TransferSettingsContent =
 new GUIContent("Transfer Settings", "Transfer your settings from old Asset Hunter into PRO");
            ...
            if (infoLoaded && m_TreeView.GetCombinedUnusedSize() > 0)
            {
                ...
                m_TreeView.DrawDeleteAllButton(instancedGUIContent, newStyle,
                    GUILayout.MaxHeight(AH_SettingsManager.Instance.HideButtonText ? btnMaxHeight * 2f : btnMaxHeight));
            }
        ...
        {
            if (EditorUtility.DisplayDialog("新构建", "确定要根据当前的构建设置创建新的构建?", "确定",
                "取消"))
            {
        ...
        {
            string fileSelected = EditorUtility.OpenFilePanel("", AH_SerializationHelper.GetBuildInfoFolder(),
        ...
        Rect toolbarRect
        {
            get
            {
                return new Rect(UiStartPos.x, UiStartPos.y + (AH_SettingsManager.Instance.HideButtonText ? 20 : 0),
                    position.width - (UiStartPos.x * 2), 20f);
            }
        }
        ...
        Rect multiColumnTreeViewRect
        {
            get
            {
                return new Rect(UiStartPos.x, UiStartPos.y + 20 + (AH_SettingsManager.Instance.HideButtonText ? 20 : 0),
        ...
        {
            get { return uiStartPos; }

            set { uiStartPos = value; }
        }
        ...
        void doBottomToolBar(Rect rect)
        {
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                if (((AH_MultiColumnHeader) m_TreeView.multiColumnHeader).mode ==
                    AH_MultiColumnHeader.Mode.SortedList || !string.IsNullOrEmpty(m_TreeView.searchString))
                {
                ...
                GUIContent exportContent = new GUIContent("Export list",
                    "Export all the assets in the list above to a json file");
