...
{
    [InitializeOnLoad]
    public sealed class HierarchyProEditor
    {
        public static EditorApplication.HierarchyWindowItemCallback hierarchyproWindowItemOnGUI;
        internal const int GLOBAL_SPACE_OFFSET_LEFT = 16 * 2;

        static HierarchyProEditor instance;

        public static HierarchyProEditor Instance
        {
            get
            {
                if (instance == null)
                    instance = new HierarchyProEditor();
                return instance;
            }
        ...
        static HierarchyProEditor()
        {
            if (instance == null)
                instance = new HierarchyProEditor();
        }

        public HierarchyProEditor()
        {
            InternalReflection();
        ...
        static void InternalReflection()
        {
            ...
            MethodInfo OnRepaintHierarchyWindow = typeof(HierarchyProEditor).GetMethod(nameof(OnRepaintHierarchyWindow),
            ...
            FieldInfo windowsReordered = typeof(EditorApplication).GetField(nameof(windowsReordered),
                BindingFlags.Static | BindingFlags.NonPublic);
            MethodInfo OnWindowsReordered = typeof(HierarchyProEditor).GetMethod(nameof(OnWindowsReordered),
                BindingFlags.NonPublic | BindingFlags.Static);
            ...
            if (checkingAllHierarchy == true)
            {
                ...
                foreach (EditorWindow window in GetAllSceneHierarchyWindowsDelegate())
                {
                    if (!HierarchyWindow.instances.ContainsKey(window.GetInstanceID()))
                    {
                        ...
                        hierarchyWindow.SetWindowTitle("Hierarchy Pro");
                    }
        ...
        void HierarchyOnGUI(int selectionID, Rect selectionRect)
        {
            ...
            hierarchyproWindowItemOnGUI?.Invoke(selectionID,selectionRect);
        }
        ...
        internal sealed class MenuCommand
        {
            ...
            [MenuItem("GameObject/上锁选项  %l", false, priority)]
            static void SetNotEditableObject()
            {
                Undo.RegisterCompleteObjectUndo(Selection.gameObjects, "设置选择标志不可编辑");
            ...
            [MenuItem("GameObject/上锁选项 %l", true, priority)]
            static bool ValidateSetNotEditableObject() => Selection.gameObjects.Length > 0;

            [MenuItem("GameObject/解锁选项 %&l", false, priority)]
            static void SetEditableObject()
            {
                Undo.RegisterCompleteObjectUndo(Selection.gameObjects, "设置选择标志可编辑");
                for (int i = 0; i < Selection.gameObjects.Length; ++i)
                {
            ...
            [MenuItem("GameObject/解锁选项 %&l", true, priority)]
            static bool ValidateSetEditableObject() => Selection.gameObjects.Length > 0;


            [MenuItem("GameObject/上移选择 #w", false, priority)]
            ...
            [MenuItem("GameObject/上移选择 #w", true)]
            static bool ValidateQuickSiblingUp() => Selection.activeTransform != null;

            [MenuItem("GameObject/下移选择 #s", false, priority)]
            ...
            [MenuItem("GameObject/下移选择 #s", true, priority)]
            static bool ValidateQuickSiblingDown() => Selection.activeTransform != null;

            [MenuItem("GameObject/创建 标题分隔符", priority = 0)]
            static void CreateHeaderInstance() => new GameObject(string.Format("{0}NewHeader", HierarchyProEditor.instance.settings.headerPrefix));
        }
    }
}